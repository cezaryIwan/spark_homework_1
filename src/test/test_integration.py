...
def test_run_etl_pipeline(spark, test_data_dir):
    ...
    df_hotels = spark.read.option('header', True).csv(test_data_dir['hotels_path'])

    assert len(result_dicts) == df_hotels.count()

    expected_columns = {'hotel_id', 'city', 'lat', 'lng', 'avg_tmpr_f', 'avg_tmpr_c', 'wthr_date', 'geohash'}
    for col in expected_columns:
        assert col in result_dicts[0]
